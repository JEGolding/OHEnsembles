...
for dataset in ['mnist','fashion_mnist','kmnist']:
    ...
    for rem in ['None']+list(range(10)):
        ...
        # Train
        np.save(f'{dataset}/{rem}/embedding.npy', np.array(embedding))
        np.save(f'{dataset}/{rem}/labels.npy', np.array(clusterer.labels_))
        
        # Test
        np.save(f'{dataset}/{rem}/test_embedding.npy', np.array(test_embedding))
        np.save(f'{dataset}/{rem}/test_labels.npy', np.array(test_labels))
        
        # Classifier results
        # Prediction vectors from classifiers
        np.save(f'{dataset}/{rem}/pred_vector.npy', np.array(pred_vector))
        np.save(f'{dataset}/{rem}/train_pred_vector.npy', np.array(train_pred_vector))
        
        # OHE distances
        np.save(f'{dataset}/{rem}/dists.npy', dists)
        np.save(f'{dataset}/{rem}/train_dists.npy', train_dists)